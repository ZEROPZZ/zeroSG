...
class MultiModalFramework(nn.Module):
    ...
    def forward(self, image, text, audio):
        ...
        vqa_output = self.vqa_layer(fused_features) if self.config.enable_vqa else None
        caption_output = self.caption_layer(fused_features) if self.config.enable_caption else None
        retrieval_output = self.retrieval_layer(fused_features) if self.config.enable_retrieval else None
        asr_output = self.asr_layer(fused_features) if self.config.enable_asr else None
        realtime_asr_output = self.realtime_asr_layer(fused_features) if self.config.enable_realtime_asr else None
...
# 进行推理
with torch.no_grad():
    # 假设 image, text, audio 已经定义
    vqa_output, caption_output, retrieval_output, asr_output, realtime_asr_output = model(image, text, audio)

# 打印输出（省略，假设已经定义）

# 保存模型
torch.save(model, 'sg_model.pth')

# 加载模型
loaded_model = torch.load('sg_model.pth')
loaded_model.eval()  # 设置为评估模式

# 使用加载的模型进行推理
with torch.no_grad():
    vqa_output_loaded, caption_output_loaded, retrieval_output_loaded, asr_output_loaded, realtime_asr_output_loaded = loaded_model(image, text, audio)

# 打印加载模型的输出
print("Loaded VQA Output:", vqa_output_loaded)
print("Loaded Caption Output:", caption_output_loaded)
print("Loaded Retrieval Output:", retrieval_output_loaded)
print("Loaded ASR Output:", asr_output_loaded)
print("Loaded Realtime ASR Output:", realtime_asr_output_loaded)
